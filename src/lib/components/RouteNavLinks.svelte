<script lang="ts">
	...
	import type { RouteNavContext } from '$lib/utils/routeNav';

	type RouteNavItem = {
		id: RouteNavContext['current'];
		href: string;
		label: string;
		compactLabel?: string;
	};
	...
	let {
		...
		threadUrl = null,
		handle = null,
		dialogueHandleA = null,
		dialogueHandleB = null,
		hideThreadTools = false
	...
	const items: RouteNavItem[] = [
		...
		{ id: 'handtracker', href: '/handtracker', label: 'Hand Tracker' },
		{ id: 'handtrainer', href: '/handtrainer', label: 'Hand Trainer' },
