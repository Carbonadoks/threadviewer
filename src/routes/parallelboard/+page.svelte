...
<svelte:head>
	<title>Parallel Board</title>
</svelte:head>

<main style="font-family: {fontFamily}">
	<header>
		<RouteNav
			current="parallelboard"
			align="center"
			threadUrl={urlInput}
			handle={parseBskyPostUrl(urlInput)?.handle ?? null}
		/>
		<h1>Parallel Board</h1>
		<p class="subtitle">Paste a Bluesky post URL to view its thread on the parallel board</p>
