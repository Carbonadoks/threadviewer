...
async function main() {
	console.log('Run this builder with: npm run cluster:build');
	...
	try {
		console.log(`Building cluster snapshot in bucket ${bucket}.`);
		const snapshot = await buildOfflineClusterSnapshot({
			storage,
			fetchEnabled,
		...
		console.log(
			`Snapshot ready: ${snapshot.meta.totalThreads} threads, ${snapshot.meta.clusterCount} clusters, ${Math.round(snapshot.meta.projectionNeighborRecall * 100)}% 10-NN preservation.`
		);
		process.exitCode = 0;
	} catch (error: any) {
		console.error(error?.message || 'Cluster snapshot build failed.');
		if (error?.stack) {
